...
try:
    models = [NeuralNetwork.load_old('models/d/d-trained-depth-'+str(i)) for i in layers]
except FileNotFoundError:
    def train_for_unit(l, X_train, y_train):
        l = [(i,'relu') for i in l]
        nn = NeuralNetwork(n=3072, layers=l, r=(n_classes, 'softmax'))
        nn.fit(X_train, y_train, lr=1e-2, batch_size=32, tol=1e-4, verbose=3, epochs=70, out_path=f'plots/d/epoch-vs-loss-{len(l)}-layers.png')
        return nn
    ...
    [models[i].save('models/d/d-trained-depth-'+str(l)) for i,l in enumerate(layers)]
...
plt.legend()
plt.tight_layout()
out_path = 'plots/d/depth-vs-f1score.png'
...
submission.to_csv(args.out+'/precitions_d.csv', index=False)